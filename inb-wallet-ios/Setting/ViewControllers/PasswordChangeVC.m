...
-(void)doneAction:(UIButton *)sender{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if(!wallet){
                [MBProgressHUD showMessage:NSLocalizedString(@"password.changed.failed.oldError",@"原密码错误") toView:self.view afterDelay:1 animted:YES];
            }else{
                [MBProgressHUD showMessage:NSLocalizedString(@"password.changed.success",@"密码修改成功") toView:self.view afterDelay:1 animted:YES];
